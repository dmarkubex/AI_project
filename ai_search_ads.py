...
def main():
    ...
    with col2:
        ...
        selected_suggestion = st.radio("选择一个建议", suggestions)

        if st.button("使用建议搜索"):
            # 使用搜索建议的逻辑
            st.write(f"根据建议 '{selected_suggestion}' 的搜索结果...")

            # 示例：直接使用建议作为查询
            query_vector = get_embedding(selected_suggestion)
            if query_vector:
                search_results = vector_search(query_vector)
                if search_results:
                    reranked_results = rerank(selected_suggestion, search_results)
                    for result in reranked_results:
                        st.write(result)
